...
namespace fcitx {
    ...
    void LotusEngine::keyEvent(const InputMethodEntry& /*entry*/, KeyEvent& keyEvent) {
        ...
        if (isShowingQuickToggle_) {
            ...
            auto   moveCursor = [&](int delta) {
                ...
                int pageSize   = menuList->pageSize();
                int targetPage = (pageSize > 0) ? nextIndex / pageSize : 0;
                if (menuList->currentPage() != targetPage) {
                    menuList->setPage(targetPage);
                }
                ic->updateUserInterface(UserInterfaceComponent::InputPanel);
                return true;
            };
            ...
            switch (keySym) {
                ...
                case FcitxKey_Right: {
                    if (menuList && menuList->totalPages() > 1) {
                        int currentPage = menuList->currentPage();
                        int totalPages  = menuList->totalPages();
                        if (currentPage + 1 < totalPages) {
                            menuList->setPage(currentPage + 1);
                            ic->updateUserInterface(UserInterfaceComponent::InputPanel);
                        }
                    }
                    return;
    ...
    void LotusEngine::showQuickToggleMenu(InputContext* ic) {
        ...
        std::unordered_map<std::string, ToggleEntry> entryMap = {
            ...
            {"AutoCapitalize", {&config_.autoCapitalizeAfterPunctuation, Key(*config_.shortcutToggleAutoCapitalize).sym(), nullptr, "", _("Auto Capitalize After Punctuation")}},
            {"DoubleHyphen", {&config_.doubleHyphenToEmDash, Key(*config_.shortcutToggleDoubleHyphen).sym(), nullptr, "", _("Double Hyphen to Em-Dash")}},
            {"FixUinput", {&config_.fixUinputWithAck, Key(*config_.shortcutToggleFixUinput).sym(), nullptr, "", _("Fix Uinput With Ack")}},
        };

        std::unordered_map<std::string, Option<bool>*> visibilityMap = {
            {"SpellCheck", &config_.showToggleSpellCheck},       {"Macro", &config_.showToggleMacro},
            {"AutoRestore", &config_.showToggleAutoRestore},     {"FixStickyShift", &config_.showToggleFixStickyShift},
            {"CustomDictionary", &config_.showToggleDictionary}, {"CapitalizeMacro", &config_.showToggleCapitalizeMacro},
            {"ModernStyle", &config_.showToggleModernStyle},     {"FreeMarking", &config_.showToggleFreeMarking},
            {"DoubleSpace", &config_.showToggleDoubleSpace},     {"AutoCapitalize", &config_.showToggleAutoCapitalize},
            {"DoubleHyphen", &config_.showToggleDoubleHyphen},   {"FixUinput", &config_.showToggleFixUinput},
        };
        ...
        ic->inputPanel().setAuxUp(Text(_("Up/Down/Tab: Move  Left/Right: Page  Key: Quick Toggle")));
        ic->updateUserInterface(UserInterfaceComponent::InputPanel);
    }
